...
def show_inference(model, frame_np):
    # Actual detection.
    output_dict = run_inference_for_single_image(model, frame_np)
    ...
    vis_util.visualize_boxes_and_labels_on_image_array(
        ...
        line_thickness=8)
    cv.imshow("Car Detection", frame_np)


while True:
    ret, frame_np = vid.read()
    show_inference(detection_model, frame_np)
    key = cv.waitKey(30)
    if key == ord('q'):
        break
